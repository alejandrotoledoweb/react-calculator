...
describe('Calculate', () => {
  it('it test the addition of numbers', () => {
    ...
    expect(total).toEqual({ total: null, next: '7', operation: null });
  });

  it('it test the substract of numbers', () => {
    const total = calculate({ total: '8', next: '5', operation: '−' }, '=');
    expect(total).toEqual({ total: null, next: '3', operation: null });
  });
  ...
  it('it convert the number into a percent number', () => {
    ...
    expect(total).toEqual({ total: null, next: '0.4', operation: null });
  });
});
